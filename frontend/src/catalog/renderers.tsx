...
const demonstrationCatalogRenderers: CatalogRenderers<DemonstrationCatalogDefinitions> = {
  ...
  Badge: ({ props }) => {
    ...
    return (
      <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 9999, fontSize: "0.7rem", fontWeight: 500, background: v.bg, color: v.color }}>
        {resolveText(props.text)}
      </span>
    );
  },
            ...
            {rows.map((row: any, i: number) => (
              <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
                {cols.map((col: any) => {
                  ...
                  const isErrorCol = col.key.toLowerCase().includes("error");
                  return (
                    <td key={col.key} style={{ padding: "8px 12px", color: isErrorCol ? "#ef4444" : "#374151", fontFamily: isErrorCol ? "monospace" : "inherit" }}>
                      <span>{value}</span>
                      {isErrorCol && value && <CopyButton text={value} />}
                    </td>
                  );
                })}
              </tr>
  ...
  Button: ({ props, children, dispatch }) => {
    ...
    return (
      <button
        style={{ ...style, padding: "8px 16px", borderRadius: 8, fontSize: "0.8rem", fontWeight: 500, cursor: "pointer", width: "100%" }}
        onClick={() => dispatch?.(props.action)}
      >
        {typeof props.child === "string"
          ? children(props.child)
          : (props as any).label ?? null}
      </button>
    );
  },
};
